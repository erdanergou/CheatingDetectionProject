...
def box_transform(bbox, sf, imgwidth, imght, train):
    ...
    ht = bbox[3] - bbox[1]
    if train:
        # 输出后剪裁后的数组存入的数组，将数数组的结果限定在 -sf 与 sf之间
...
def addDPG(bbox, imgwidth, imght):
    """Add dpg for data augmentation, including random crop and random sample.
        添加dpg进行数据扩充，包括random crop和random sample。
    """
...
def to_numpy(tensor):
    # 将torch.tensor类型转为numpy.ndarray类型
...
def to_torch(ndarray):
    # 将numpy.ndarray转为torch.tensor类型
...
def heatmap_to_coord_simple_regress(preds, bbox, hm_shape, norm_type, hms_flip=None):
    ...
    if preds.dim() == 3:  # 如果是三维
        preds = preds.unsqueeze(0)  # 在张量最外层加一个中括号变成第一维。
